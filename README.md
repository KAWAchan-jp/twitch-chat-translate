...
- **豊富な装飾設定** — プリセット・色・間隔・表示時間・エフェクトを調整
...
| Bot・コマンド除外 | Botの発言やコマンドをチャット表示から除外 |
| 送信 [実験的] | チャット送信パネルを表示（Twitchログインが必要） |
| フォント | ゴシック体 / 明朝体 / 等幅フォントを切り替え |
| メニュー言語 | UIの表示言語を変更 |
| 装飾設定 | チャット表示のプリセットや色、エフェクトを変更 |
| 除外ユーザーを管理 | 表示しないユーザーを追加・削除 |
| OBS用URLをコピー | 現在の装飾設定を含んだOBSブラウザソース用URLをコピー |
...
| 項目 | 説明 |
|------|------|
| プリセット | Plain / Box / Separator / Bubble / Glass / Neon / Compact / Terminal / Minimal / Card / Broadcast / Pastel |
| 表示要素 | 名前・時刻・原文・翻訳の表示/非表示 |
| エフェクト | 新しいメッセージが表示されるときのモーション |
| 背景色 | すべてのプリセットに共通する発言背景色 |
| 文字色 | 翻訳・原文・名前の色を個別に変更 |
| デフォルト色に戻す | 選択中プリセット本来の配色へ戻す |
| 囲いの間隔 | 発言と発言の間隔を調整 |
| 時間が経てば消える | 指定秒数後に Fade / Slide / Shrink / Blur で発言を消去 |
| クロマキー背景色 | OBSで透過に使用する背景色 |
| カスタムCSS | CSSを直接編集して表示を細かく調整 |

設定内容はブラウザへ自動保存されます。カスタムCSSも入力時に即時保存・反映されるため、保存ボタンは必要ありません。
